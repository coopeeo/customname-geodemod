	 ...
	 bool init(int accountID, bool ownProfile) {
		...
		if (ownProfile)
		{
			...
			if (Loader::get()->isModLoaded("cvolton.betterinfo")) {
				typeinfo_cast<cocos2d::CCLabelBMFont*>(this->getChildByID("cvolton.betterinfo/username-button")->getChildren()->objectAtIndex(1))->setString(value.c_str()); //honestly i have no clue lol
			}
		}
